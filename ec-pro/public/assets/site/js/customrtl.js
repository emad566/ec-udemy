	...
	function initTabLines()
	{
		if($('.tabs').length)
		{
			...
			tabs.each(function()
			{
                ...
                // var posX = ($(window).width() - ( $(tabGroup[0]).outerWidth()));
                // alert($(tabGroup[0]).offset().right)
                // alert(posX)
                var posX = $(tabGroup[0]).position().left + parseInt($(tabGroup[0]).css('margin-left'))
				tabsLine.css({'left': posX, 'right':'auto', width: $(tabGroup[0]).width()});
				tabGroup.each(function()
				{
					...
					tab.on('click', function()
					{
						if(!tab.hasClass('active'))
						{
                            ...
                            // var tabXPos = ($(window).width() - ( tab.outerWidth()));
							var tabXPos = tab.position().left + parseInt(tab.css('margin-left')) +tabWidth;
